                    ...
                    series: [
                        {
                            ...
                            data: [
                                ...
                                {
                                    name: `任务数\n${this.tasks.filter(y=>this.projects.filter(x=>x.owner_user_name===team.global_key).map(x=>x.id).includes(y.project_id)).length}`,
                                    x: 400,
                                    y: 300,
                                    itemStyle: {
                                        normal: {
                                ...
                                {
                                    name: `已完成任务数\n${this.tasks.filter(y=>this.projects.filter(x=>x.owner_user_name===team.global_key).map(x=>x.id).includes(y.project_id) && y.status===2 ).length}`,
                                    x: 500,
                                    y: 150,
                                    itemStyle: {
                                        normal: {
                                ...
                                {
                                    name: `未完成任务数\n${this.tasks.filter(y=>this.projects.filter(x=>x.owner_user_name===team.global_key).map(x=>x.id).includes(y.project_id) && y.status===1 ).length}`,
                                    x: 600,
                                    y: 340,
                                    itemStyle: {
                                        normal: {
                                ...
                                {
                                    name: `重要且紧急任务\n${this.tasks.filter(y=>this.projects.filter(x=>x.owner_user_name===team.global_key).map(x=>x.id).includes(y.project_id) && y.status===1 && y.priority===3 ).length}`,
                                    x: 640,
                                    y: 230,
                                    itemStyle: {
                                        normal: {
                                ...
                                {
                                    name: `重要不紧急任务\n${this.tasks.filter(y=>this.projects.filter(x=>x.owner_user_name===team.global_key).map(x=>x.id).includes(y.project_id) && y.status===1 && y.priority===2 ).length }`,
                                    x: 740,
                                    y: 310,
                                    itemStyle: {
                                        normal: {
                                ...
                                {
                                    name: `紧急不重要任务\n${this.tasks.filter(y=>this.projects.filter(x=>x.owner_user_name===team.global_key).map(x=>x.id).includes(y.project_id) && y.status===1 && y.priority===1 ).length }`,
                                    x: 750,
                                    y: 430,
                                    itemStyle: {
                                        normal: {
                                ...
                                {
                                    name: `不紧急不重要任务\n${this.tasks.filter(y=>this.projects.filter(x=>x.owner_user_name===team.global_key).map(x=>x.id).includes(y.project_id) && y.status===1 && y.priority===0 ).length}`,
                                    x: 640,
                                    y: 500,
                                    itemStyle: {
                                        normal: {
